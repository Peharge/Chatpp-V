...
class ChatApp(QWidget):
    ...
    def initUI(self):
        ...
        # Pfad zur SVG-Datei
        icon_pathxpp = 'C:\\Users\\julia\\PycharmProjects\\peharge-chatpp-pro\\xpp.svg'

        chatxpp_button = QPushButton()

        # Icon laden und auf die gewünschte Größe skalieren
        icon_pixmap = QPixmap(icon_pathxpp).scaled(70, 70)
        icon = QIcon(icon_pixmap)
        chatxpp_button.setIcon(icon)

        # Hinzufügen eines Schatteneffekts zum Button
        shadow_effect = QGraphicsDropShadowEffect(chatxpp_button)
        shadow_effect.setBlurRadius(10)
        shadow_effect.setColor(Qt.black)  # Schwarze Farbe für den Schatten
        shadow_effect.setOffset(5, 5)
        chatxpp_button.setGraphicsEffect(shadow_effect)
        chatxpp_button.setFixedSize(75, 35)
        chatxpp_button.setIconSize(QtCore.QSize(21, 21))  # Hier Größe der SVG-Datei angeben
        chatxpp_button.setIconSize(QSize(21, 21))  # Hier Größe der SsVG-Datei angeben
        chatxpp_button.setFont(QFont('Arial', 12, QFont.Bold))
        chatxpp_button.setStyleSheet("""
            QPushButton {
                background-color: #1c1c1c;
                color: #ffffff;
                border-radius: 5px;
        ...
        # Füge den special_char_button zum Layout hinzu mit einem Abstand von 20px oben.
        button_layout.addWidget(chatxpp_button)
        chatxpp_button.clicked.connect(self.run_xpp)

        def move_chatxpp_button_up():
            chatxpp_button.move(chatxpp_button.x(), chatxpp_button.y() - 5)

        # Definiere eine Funktion, um den Button um 10px nach unten zu verschieben.
        def move_chatxpp_button_down(button):
            if not button.isChecked():
                button.move(button.x(), button.y() + 5)

        # Verbinde die Funktionen mit den entsprechenden Signalen.
        chatxpp_button.pressed.connect(move_chatxpp_button_up)
        chatxpp_button.released.connect(lambda: move_chatxpp_button_down(chatxpp_button))

        # Verbinde die enterEvent und leaveEvent mit den Funktionen.
        chatxpp_button.enterEvent = lambda event: move_chatxpp_button_up()
        chatxpp_button.leaveEvent = lambda event: move_chatxpp_button_down(chatxpp_button)

        self.setLayout(button_layout)

        # Pfad zur SVG-Datei
        icon_pathvg = 'C:\\Users\\julia\\PycharmProjects\\peharge-chatpp-pro\\xcpp-img2.svg'
    ...
    def process_input(self):
        ...
        if text_input:
            ...
            command = "ollama run llama3.1" # ändern durch Chatpp5
    ...
    def run_xpp(self):
        os.system('C:\\Users\\julia\\PycharmProjects\\X++\\run-model.bat')
